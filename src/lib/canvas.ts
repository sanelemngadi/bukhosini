import { ESprotKeyState, SprotPoint, SprotSelectionWrapper, type SprotAppViewController } from "$wasm/sprot_app";
...
export class SprotCanvas {
    ...
    constructor(
        ...
        private _horizontalRulersCanvas: HTMLCanvasElement,
        private _verticalRulersCanvas: HTMLCanvasElement,
    ...
    addSelection(selection: SprotSelectionWrapper): SprotCanvas {
        if(this._targetTool && this._appState) {
            this._targetTool.setSelection(this._appState, selection);
        }
        return this;
    }

    Resize = (appState: SprotAppViewController, w: number, h: number): SprotCanvas => {
        const scale = devicePixelRatio;

        // console.log("w: ", w, " h: ", h, " scale: ", devicePixelRatio);
        
        const rulersSize = 16.0 * scale;

        this._canvas.width = w * scale;
        this._staticCanvas.width = w * scale;
        this._horizontalRulersCanvas.width = w * scale;
        this._verticalRulersCanvas.width = rulersSize;

        this._canvas.height = h * scale;
        this._staticCanvas.height = h * scale;
        this._verticalRulersCanvas.height = h * scale;
        this._horizontalRulersCanvas.height = rulersSize;

        this._canvas.style.width = 100 + "%";
        this._staticCanvas.style.width = 100 + "%";
        this._horizontalRulersCanvas.style.width = 100 + "%";
        this._verticalRulersCanvas.style.width = "100%";

        this._canvas.style.height = 100 + "%";
        this._staticCanvas.style.height = 100 + "%";
        this._verticalRulersCanvas.style.height = 100 + "%";
        this._horizontalRulersCanvas.style.height = 100 + "%";


        // container.style.border = "1px solid red";


        // canvas.style.width = parentDiv.offsetWidth + 'px';
        // canvas.style.height = parentDiv.offsetHeight + 'px';
        // staticCanvas.style.width = parentDiv.offsetWidth + 'px';
        // staticCanvas.style.height = parentDiv.offsetHeight + 'px';

        if (appState) {
            // let size = new SprotSize(canvas.width, canvas.height);

            // console.log("w: ", parentDiv.offsetWidth, " h: ", parentDiv.offsetHeight);
            // console.log("screen avail w: ", screen.availWidth, " h: ", screen.availHeight);
    ...
    setAppController(appState: SprotAppViewController): SprotCanvas {
        this._appState = appState;
        return this;
    }

    setActionTool(tool: SprotCanvasTool): SprotCanvas {
        this._targetTool = tool;
        return this;
    }

    setActionModifierTool(tool: SprotCanvasModifierTool | null): SprotCanvas {
