<script lang="ts">
  ...
  import SearchLayer from "./SearchLayer.svelte";
  import type { SprotClientDocument } from "$lib/application/document";
  import { SprotLayer } from "$wasm/sprot_app";
...
<div class="border h-full flex flex-col overflow-hidden border-sprotBgLight60">
  <SearchLayer />
    ...
    <div class="borderl border-sprotBgLight60">
      {#if document}
        <ul>
          {#each layers as lyr}
            <li
              class="h-6 border border-transparent hover:border-sprotPrimary flex items-center group"
            >
              ...
              <DullButton className="w-5 borderk h-full sprot-center">
                <div class="flex gap-1 items-center h-4 cursor-pointer">
                  <div
                    class="flex items-center justify-center border pointer-events-none w-3 h-3 bg-sprotBg1"
                    style="background-color: {lyr.get_stroke_color()};"
                  >
                    <div class="flex w-[5px] h-[5px] border bg-sprotBg1"></div>
                  </div>
                </div>
              </DullButton>
              ...
              <input
                ...
                id={String(lyr.id)}
                class="h-full px-1 disabled:bg-sprotBg1 flex-1 w-20"
                value={lyr.get_name()}
                disabled={true}
