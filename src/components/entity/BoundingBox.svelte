  ...
  import { NO_SELECTION, type SprotTSSelection } from "$lib/types";
    ...
    const init = () => {
        getCurrentSelection(selected => {
            ...
            // if
            // selected.Empty(onClearSelection)
            // .GroupAll(onClearSelection)
            // .Single(onSetSelection);
        });
    }
...
<div class="grid grid-cols-2 gap-2">
    <EditableInput initValue={selectedEntity ? toFixed(selection.rect.pt.x) : "mixed"}>X</EditableInput>
    <EditableInput initValue={selectedEntity ? toFixed(selection.rect.pt.y) : "mixed"}>Y</EditableInput>
    <EditableInput initValue={selectedEntity ? toFixed(selection.rect.sz.w) : "mixed"}>W</EditableInput>
    <EditableInput initValue={selectedEntity ? toFixed(selection.rect.sz.h) : "mixed"}>H</EditableInput>
    <EditableInput initValue="00.00.00"><SnapPerpendicular size={16} color="white"/></EditableInput>
</div>