...
<DullButton
  className="sprot-layers relative border flex items-center justify-center border-sprotBg1 group bg-sprotBg hover:bg-sprotBg1 h-6 overflow-hidden group min-w-48 w-48 max-w-48 {active &&
    'bg-sprotBg1'} {disabled &&
    'bg-sprotBg text-sprotBgLight60 pointer-events-none'}"
...
>
  <div class="w-full h-full" bind:this={container}>
    ...
    >
      {#if activeChoice}
        {ellipses(activeChoice.label, 20)}
      {/if}
    </div>
          ...
          <div class="flex items-center h-full gap-1">
            <span
              class="w-1 h-1 bg-sprotPrimary {!choice.active &&
                'invisible opacity-0'}"
            ></span>
            <p>{ellipses(choice.label, 20, "CENTER")}</p>
          </div>
        </DullButton>
      {/each}
    </ul>
